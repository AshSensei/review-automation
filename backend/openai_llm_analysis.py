...
CORS(
    app,
    resources={r"/api/*": {"origins": [r"https://.*\.vercel\.app$", "http://localhost:3000"]}},
...
def parse_reviews_bs4(html: str) -> List[Dict]:
    ...
    try:
        ...
        review_elements = soup.find_all(['li', 'div'], class_=re.compile(r'review|comment|feedback', re.I))
        ...
        for elem in review_elements[:PARSE_HTML_LIMIT]:
            ...
            if text_elem:
                text = text_elem.get_text() if hasattr(text_elem, 'get_text') else str(text_elem)
                text = text.strip()[:2000]
                
                if len(text) > 20:  # Minimum review length
                    # Extract rating
                    rating = None
                    rating_elem = elem.find(string=re.compile(r'Rated (\d(?:\.\d)?)', re.I))
                    if rating_elem:
    ...
    try:
        ...
        for event, elem in context_iter:
            ...
            if elem.tag == 'li':
                ...
                txt = text_elem.text.strip()[:2000] if text_elem is not None and text_elem.text else ''
...
def analyze_html():
    try:
        ...
        response = {
            'success': True,
            'reviews_sample': [r['review_text'][:500] for r in reviews[:20]],
